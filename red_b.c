// red_b.c — Bootscan-regularized Recombination-aware Edit Distance (RED-B)
// Author: Katsuhiko Hayashi
// Build:   gcc -O3 -std=c11 -o red_b red_b.c
//
// Usage:
//   ./red_b --parents-aln parents.aligned.fasta --child child.fasta \
//           --out out/prefix \
...
static void usage(){
  fprintf(stderr,
    "Usage: red_b --parents-aln PARENTS.fasta --child CHILD.fasta --out OUTPREFIX\n"
    "             [--mismatch 1] [--gap 2] [--switch 1]\n"
    "             [--win 21] [--kappa 1] [--stick 0]\n"
    "             [--mu-inf <int>] [--mu-eq <int>]\n"
    "             [--gap-swell 1] [--gap-near 10] [--reward 0]\n");
...
　　snprintf(path1,sizeof(path1), "%s.%s.origins.tsv",  outprefix, "red");
    ...
    fprintf(stderr,"[OK] red_b %s  total=%d  mis=%d ins=%d del=%d sw=%d  (end=%c)  W=%d kappa=%d stick=%d mu_inf=%d mu_eq=%d\n",
